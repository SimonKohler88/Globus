...
/* USER CODE BEGIN 1 */
void adc_init( TRIPLE_ADC_t* triple_adc, ADC_HandleTypeDef* hadc ) { triple_adc->hadc = hadc; }
void adc_update( TRIPLE_ADC_t* triple_adc )
{
    for ( uint8_t i = 0; i < 3; i++ )
    {
        HAL_ADC_Start( triple_adc->hadc );
        HAL_ADC_PollForConversion( triple_adc->hadc, 1 );
        uint32_t adc_val = HAL_ADC_GetValue( triple_adc->hadc );
        switch ( i )
        {
            case 0 :
