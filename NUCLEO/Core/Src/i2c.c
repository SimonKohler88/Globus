...
void i2c_init( I2C_DATA_MEM_t* data_mem )
{
    ...
    data_storage->num_entries = I2C_NUM_ADDRESS;
...
extern void HAL_I2C_AddrCallback( I2C_HandleTypeDef* hi2c, uint8_t TransferDirection, uint16_t AddrMatchCode )
{
    ...
    {
        if ( data_storage->data[ 0 ] < I2C_NUM_ADDRESS )
        {
            uint32_t reg_2_read = data_storage->data[ 0 ];
            HAL_I2C_Slave_Transmit_IT( hi2c, ( uint8_t* ) data_storage->data[ reg_2_read ], 4 );
        }
    }
    else
    {
...
extern void HAL_I2C_ListenCpltCallback( I2C_HandleTypeDef* hi2c )
{
    ...
    if ( addr < I2C_NUM_ADDRESS )
    {
        uint32_t val = RxData[ 1 ] << 24 | RxData[ 2 ] << 16 | RxData[ 3 ] << 8 | RxData[ 4 ];
        if ( addr != 0 )
        {
            data_storage->data[ addr ] = val;
            data_storage->has_update   = 1;
        }
        else if ( val < I2C_NUM_ADDRESS) data_storage->data[ addr ] = val;
    }
    // HAL_I2C_ClearFlag(I2C_Module, I2C_FLAG_BERR);
}
extern void HAL_I2C_TxCpltCallback( I2C_HandleTypeDef* hi2c )
{
    HAL_I2C_EnableListen_IT( hi2c );
}
...
uint32_t i2c_read( const uint8_t address )
{
    uint32_t val = 0;
    if ( address < I2C_NUM_ADDRESS )
    {
        HAL_NVIC_DisableIRQ( I2C1_EV_IRQn );
        HAL_NVIC_DisableIRQ( I2C1_ER_IRQn );

        val = data_storage->data[ address ];
